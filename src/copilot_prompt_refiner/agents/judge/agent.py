...
from copilot_prompt_refiner.agents.judge.definition import (
    LLM_REVIEW_SYSTEM_PROMPT,
    MAX_ITERS_SUMMARY_SYSTEM_PROMPT,
    TIE_BREAK_SYSTEM_PROMPT,
    build_llm_review_instruction,
    build_max_iters_instruction,
    build_tie_break_instruction,
)
from copilot_prompt_refiner.agents.judge.tools import (
    as_str_list,
    clamp_0_to_10,
    evidence_for_tag,
    extract_json_object,
    metric_to_failure_tag,
    normalize_tags,
    required_fix_for_tag,
    success_criteria_for_tag,
    target_section_for_tag,
)
...
class JudgeAgent:
    ...
    def summarize_max_iters(self, final_judge: JudgeResult, max_iters: int) -> str:
        ...
        instruction = build_max_iters_instruction(max_iters=max_iters, final_judge=final_judge)
        try:
            summary = self.runtime.complete(
                system_prompt=MAX_ITERS_SUMMARY_SYSTEM_PROMPT,
    ...
    def _llm_review(
        ...
        *,
        ...
        """
        instruction = build_llm_review_instruction(
            candidate_prompt=candidate_prompt,
    ...
    def _tie_break_with_runtime(
        ...
        *,
        ...
        instruction = build_tie_break_instruction(
            case_id=case.case_id,
            aggregated=aggregated,
            failure_cases=failure_cases,
            prioritized_actions=prioritized_actions,
        ...
        raw = self._runtime_complete_for_model(
            model=model,
            system_prompt=TIE_BREAK_SYSTEM_PROMPT,
            user_input=instruction,
        )
        parsed = self._extract_json(raw)
    ...
    def _metric_to_failure_tag(self, metric: str) -> str:
        ...
        """
        return metric_to_failure_tag(metric)
    ...
    def _required_fix_for_tag(self, tag: str) -> str:
        ...
        """
        return required_fix_for_tag(tag)
    ...
    def _target_section_for_tag(self, tag: str) -> str:
        ...
        """
        return target_section_for_tag(tag)
    ...
    def _success_criteria_for_tag(self, tag: str) -> str:
        ...
        """
        return success_criteria_for_tag(tag)
    ...
    def _evidence_for_tag(self, tag: str, metric_reasoning: dict[str, str]) -> str:
        ...
        """
        return evidence_for_tag(tag, metric_reasoning)
